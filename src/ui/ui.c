...
void draw_playfield(GameData *game)
{
    ...
    for (int row = 0; row < PLAYFIELD_SIZE; row += PLAYFIELD_COLUMNS)
    {
        ...
        for (int column = row % PLAYFIELD_COLUMNS; column < PLAYFIELD_COLUMNS; ++column)
        {
            wattron(game->ui.playfieldWindow, COLOR_PAIR(game->playfield[row + column]));
            mvwprintw(game->ui.playfieldWindow, screenRow, screenColumn, "  ");
            wattroff(game->ui.playfieldWindow, COLOR_PAIR(game->playfield[row + column]));
            screenColumn += 2;
        }
    }
